...
int main()
{
  ...
  float real_w[(int)nofbin][3]; float real_obs[(int)nofbin][3];
  ...
  sum[0] = 0; sum[1] = 0; sum[2];
  file = fopen("33real.txt","r");
  int ctr = 0;
  for (int i=0; i<nofbin; i++){
    fscanf(file,"%f %f %f \n",*(real_w+i),*(real_w+i)+1,*(real_w+i)+2);
    sum[0] += **(real_w+i);
    sum[1] += *(*(real_w+i)+1);
    sum[2] += *(*(real_w+i)+2);
  }
  ...
  for(int q=0; q<3; q++){
	...
	for(int l=0; l<nofbin; l++){
	  model[i][j][l] = sawtooth(k*tan(theta[i][j])*cos(l*2*PI/256.-phi[i][j])+(2/3*offset+q)*PI,PI);
