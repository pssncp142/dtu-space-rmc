...
int main() 
{
  ...
  for(int i=0; i<lastndx; i++){
    ...
    countA[i] = sawtooth(PIL_over_d*tan(theta)*cos(i*2*PI/nofbins-phi)+0.4*offset*PI,PI)*randphot[(int) floor(rnd/probint)];
    countB[i] = sawtooth(PIL_over_d*tan(theta)*cos(i*2*PI/nofbins-phi)+(0.4*offset+1)*PI,PI)*randphot[(int) floor(rnd/probint)];
    countC[i] = sawtooth(PIL_over_d*tan(theta)*cos(i*2*PI/nofbins-phi)+(0.4*offset+2)*PI,PI)*randphot[(int) floor(rnd/probint)];
    countD[i] = sawtooth(PIL_over_d*tan(theta)*cos(i*2*PI/nofbins-phi)+(0.4*offset+3)*PI,PI)*randphot[(int) floor(rnd/probint)];
    countE[i] = sawtooth(PIL_over_d*tan(theta)*cos(i*2*PI/nofbins-phi)+(0.4*offset+4)*PI,PI)*randphot[(int) floor(rnd/probint)];
  }  
  
  FILE* file;
  file = fopen("20real.txt","w+");
